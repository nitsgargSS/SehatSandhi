...
export default function AdminDashboard() {
  ...
  const saveVerificationNotes = async (id: string) => {
    await supabase.from('businesses').update({ verification_notes: notesDraft }).eq('id', id)
    setNotesSavedId(id)
    load()
              ...
              <div className="md:hidden space-y-3">
                {pendingCamps.map(c => (
                  <div key={c.id} className="border border-gray-100 rounded-xl p-3">
                    ...
                    <p className="text-xs text-gray-500 mb-2">{c.businesses?.name}</p>
                    <div className="text-xs text-gray-500 space-y-0.5 mb-3">
                      <div>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>{pendingCamps.map(c => (
                    <tr key={c.id} className="border-b border-gray-50 hover:bg-gray-50 transition">
                      <td className="py-3 px-2">
                        <p className="font-medium text-gray-800">{c.businesses?.name}</p>
                      </td>
