...
export default function Register() {
  ...
  const validateCoupon = async () => {
    ...
    // RLS already filters out inactive codes and anything outside
    // its valid_from/valid_until window, so a miss here means the
    // code is either wrong, expired, or not yet started.
    ...
    if (!data) {
      setCouponError(t('registerPage.couponInvalid'))
    } else if (data.max_uses !== null && (data.current_uses || 0) >= data.max_uses) {
      setCouponError(t('registerPage.couponExhausted'))
    } else {
  ...
  const discountAmount = (() => {
    ...
    if (appliedCoupon.discount_type === 'percentage')
      return Math.round(grandMonthlyEstimate * (appliedCoupon.discount_value / 100))
    if (appliedCoupon.discount_type === 'fixed_amount')
      return Math.min(appliedCoupon.discount_value, grandMonthlyEstimate)
    // free_months — the first month is fully waived
    if (appliedCoupon.discount_type === 'free_months')
      return grandMonthlyEstimate
    return 0
  })()

  const finalMonthlyEstimate = Math.max(0, grandMonthlyEstimate - discountAmount)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (appliedCoupon && newDoctor) {
        await supabase.from('discount_code_usage').insert({
          code_id: appliedCoupon.id,
        ...
        <div className="bg-teal-50 rounded-xl p-4 text-sm text-teal-700 space-y-1">
          ...
          {grandMonthlyEstimate > 0 && (
            <p className="font-bold text-base pt-2 border-t border-teal-100 mt-2">
              {t('registerPage.summaryEstTotal')}: ₹{finalMonthlyEstimate.toLocaleString('en-IN')}/mo
            </p>
          )}
        </div>
      </div>
            ...
            <div>
              ...
              <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between bg-teal-50 border border-teal-200 rounded-lg px-3 py-2">
                      <div>
                        ...
                        <p className="text-xs text-teal-600">
                          {appliedCoupon.discount_type === 'percentage' && `${appliedCoupon.discount_value}% ${t('registerPage.couponOff')}`}
                          {appliedCoupon.discount_type === 'fixed_amount' && `₹${appliedCoupon.discount_value} ${t('registerPage.couponOff')}`}
                          {appliedCoupon.discount_type === 'free_months' && t('registerPage.couponFreeMonth')}
                        </p>
                      </div>
                      <button
                        onClick={() => { setAppliedCoupon(null); setCouponInput(''); setCouponError('') }}
                        className="text-xs text-gray-400 hover:text-red-500 underline">
                        {t('registerPage.couponRemove')}
                      </button>
                    </div>
                  ) : (
                    <>
                      <div className="flex gap-2">
                        <input
                          ...
                          placeholder={t('registerPage.couponPlaceholder')}
                          value={couponInput}
                          onChange={e => { setCouponInput(e.target.value); setCouponError('') }} />
                        <button
                          onClick={validateCoupon}
                          disabled={couponChecking || !couponInput.trim()}
                          className="btn-outline text-sm px-5 shrink-0 disabled:opacity-50">
                          {couponChecking ? '...' : t('registerPage.couponApply')}
                        </button>
                      </div>
                ...
                <div className="bg-navy-700 rounded-xl p-5 text-white">
                  ...
                  {discountAmount > 0 && (
                    <div className="flex justify-between text-sm mb-2 text-teal-300">
                      <span>{t('registerPage.couponDiscountLine')} ({appliedCoupon.code})</span>
                  ...
                  <div className="border-t border-white/20 pt-3 flex justify-between font-bold text-lg">
                    ...
                    <span>₹{finalMonthlyEstimate.toLocaleString('en-IN')}</span>
                  </div>
