-- ============================================================================
-- Sehatsandhi — apply migrations 0004 to 0007 by hand
--
...
--   0007  GST on listing fees (18%, SAC 998365) and GST-compliant invoices
--
...
-- Record 0007_gst_and_invoices in the runner's ledger.
insert into public.schema_migrations (version, checksum) values
  ('0007_gst_and_invoices', 'aebce2ff3b8233e1c731d6a6df11306b5a09bd721ddbfb2a11baa7200b38f3cd')
on conflict (version) do nothing;

-- END 0007_gst_and_invoices
...
-- 6. QR entry points — consent flag true only for the opt-in ones.
select code, label, grants_marketing_consent from wa_entry_points order by code;

-- 7. GST is OFF until you fill in the details below. Expect gst_enabled = false.
select gstin, legal_name, state_code, sac_code, gst_rate, gst_enabled
  from tax_settings;

-- 8. Invoice numbering works. This BURNS a number, so only run it on sandbox.
--    select sehat_next_invoice_number();

-- ============================================================================
-- THEN, to start charging GST (fill in your real values):
--
--   update tax_settings set
--     legal_name         = '<registered legal name>',
--     gstin              = '<15-char GSTIN>',
--     state_code         = left('<15-char GSTIN>', 2),
--     state_name         = 'Haryana',
--     registered_address = '<registered address>',
--     city               = 'Yamunanagar',
--     pin_code           = '135001',
...
--   where id;
--
-- Or do the same from Admin -> Billing -> GST & invoicing, which validates the
-- GSTIN format and refuses to switch GST on without one.
-- ============================================================================
